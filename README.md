...
- **Markdown 渲染**：支援 Markdown 格式的內容渲染
...
- `GET /api/user/llm-keys/check` - 檢查用戶是否已綁定 LLM Key
- `POST /api/user/llm-keys/save` - 儲存 LLM Key
- `POST /api/user/llm-keys/test` - 測試 LLM Key
- `DELETE /api/user/llm-keys` - 刪除 LLM Key
